...
(function (nMemcached) {
  var LINEBREAK = '\r\n'
    ...
    , FLAG_BINARY = 1 << 2
    , FLAG_NUMERIC = 1 << 3
    , FLAG_ALL_TYPE = FLAG_JSON | FLAG_BINARY | FLAG_NUMERIC
    , FLAG_COMPRESS = 1 << 4;
  ...
  privates.parsers = {
    ...
    , 'SERVER_ERROR': function servererror(tokens, dataSet) {
      this.memcached.connectionIssue(tokens.splice(1).join(' '), this);
      return [CONTINUE, false];
    }
    ...
    , 'VALUE': function value(tokens, dataSet, err, queue) {
      ...
      var compress = (flag & FLAG_COMPRESS) === FLAG_COMPRESS;
      ...
      if (dataLen === '0') {
        ...
        assert(this.memcached.compressor, "No compressor provided to decompress data");
        dataSet = this.memcached.compressor.decompressSync(Buffer.from(dataSet, 'binary'));
        flag = flag & FLAG_ALL_TYPE;
        if (flag !== FLAG_BINARY) {
  ...
  privates.setters = function setters(type, validate, key, value, lifetime, callback, cas) {
    ...
    if (typeof lifetime === "function") {
      callback = lifetime;
      lifetime = this.maxExpiration;
    } else if (typeof lifetime === "object") {
      compress = lifetime.compress === true;
    ...
    if (compress) {
      flag = flag | FLAG_COMPRESS;
      assert(this.compressor, "Must provide compressor to use compression");
      value = this.compressor.compressSync(Buffer.isBuffer(value) ? value : Buffer.from(value))
      value = value.toString('binary');
    }
